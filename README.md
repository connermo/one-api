...
3. `SQL_DSN`：设置之后将使用指定数据库而非 SQLite，请使用 MySQL、PostgreSQL 或 OceanBase。
   + 例子：
     + MySQL：`SQL_DSN=root:123456@tcp(localhost:3306)/oneapi`
     + PostgreSQL：`SQL_DSN=postgres://postgres:123456@localhost:5432/oneapi`（适配中，欢迎反馈）
     + OceanBase：
       - 方式1（URL编码）：`SQL_DSN=root%40tenant%3Acluster:123456@tcp(localhost:2881)/oneapi`
       - 方式2（环境变量）：设置 `OCEANBASE_USER=root@tenant:cluster`、`OCEANBASE_PASSWORD=123456`、`OCEANBASE_HOST=localhost`、`OCEANBASE_PORT=2881`、`OCEANBASE_DATABASE=oneapi`，然后设置 `SQL_DSN=oceanbase://dummy`
       - 方式3（简单格式）：`SQL_DSN=oceanbase://root:123456@tcp(localhost:2881)/oneapi`（仅适用于用户名不含特殊字符的情况）
...
5. OceanBase 专用环境变量（当用户名包含特殊字符如冒号时推荐使用）：
   + `OCEANBASE_USER`：OceanBase 用户名（如：`root@tenant:cluster`）
   + `OCEANBASE_PASSWORD`：OceanBase 密码
   + `OCEANBASE_HOST`：OceanBase 主机地址
   + `OCEANBASE_PORT`：OceanBase 端口（默认：`2881`）
   + `OCEANBASE_DATABASE`：数据库名称
   + 使用这些环境变量时，仍需设置 `SQL_DSN=oceanbase://dummy` 来触发 OceanBase 模式
6. `FRONTEND_BASE_URL`：设置之后将重定向页面请求到指定的地址，仅限从服务器设置。
   + 例子：`FRONTEND_BASE_URL=https://openai.justsong.cn`
7. `MEMORY_CACHE_ENABLED`：启用内存缓存，会导致用户额度的更新存在一定的延迟，可选值为 `true` 和 `false`，未设置则默认为 `false`。
   + 例子：`MEMORY_CACHE_ENABLED=true`
8. `SYNC_FREQUENCY`：在启用缓存的情况下与数据库同步配置的频率，单位为秒，默认为 `600` 秒。
...
31. `TEST_PROMPT`：测试模型时的用户 prompt，默认为 `Print your model name exactly and do not output without any other text.`。
